import { THEMESTYLES } from "./modules/constants.js";
import {
  buttonSpanStyle,
  buttonStyles,
  wrapperStyles,
...
export default class SupportWidget {
  ...
  private iframeSrc = "../dist/iframe/chatbox.html";
  private eventsTracker: eventTrack[] = [];
  ...
  init(apiKey: string, config?: Partial<InitConfig>) {
    ...
    this.applyWrapperStyles(wrapper, config?.position);

    const button = this.createButton();
    this.applyButtonStyles(button, config?.buttonColor);
    ...
    this.elements.iframeWrapper = iframe;
    return this;
  }
  ...
  updateUnreadCount(count: string) {
    if (this.elements.unreadCountElement)
      this.elements.unreadCountElement.innerHTML = count;
    return this;
  }
  ...
  private createIframe() {
    ...
    iframe.style.boxShadow = "10px 10px black 10px";
